...
class Config:
    ...
    # Use Render's disk or /tmp for JSON storage
    JSON_FILE_PATH = os.getenv('JSON_FILE_PATH', '/tmp/users.json')
        ...
        for email, user_data in list(data.items()):
            ...
            if 'token' in user_data and 'token_expires_at' in user_data:
                ...
                if expires_at < now and not user_data.get('verified', False):
                    # Remove token data but keep user record
...
def create_premium_email_template(recipient: str, verification_link: str, expiry_hours: int) -> str:
    """Clean, minimal, mobile-optimized email template - No quoted text, no reply headers"""
...
def send_verification_email(recipient: str, verification_link: str) -> Tuple[bool, str]:
    """Send premium verification email via Gmail SMTP - No quoted text"""
    try:
        # Create message with proper headers to prevent quoting
        msg = MIMEMultipart('alternative')
        msg['From'] = f"{config.COMPANY_NAME} <{config.GMAIL_EMAIL}>"
        msg['To'] = recipient
        msg['Subject'] = f"Verify Your Email - {config.COMPANY_NAME}"
        # Add headers to prevent email clients from showing quoted text
        msg['X-MC-GoogleAnalytics'] = 'no'
        msg['X-Entity-Ref-ID'] = secrets.token_urlsafe(16)
        msg['Auto-Submitted'] = 'auto-generated'
        msg['Precedence'] = 'bulk'
        
        # Plain text fallback - completely fresh content, no references
...
def send_verification_link(email):
    ...
    URL Format: /send=user@example.com
    """
    # Validate email
    ...
    if success:
        return jsonify({
            ...
            'data': {
                ...
                'verification_link': verification_link  # For testing
            },
            'timestamp': datetime.now().isoformat()
        }), 200
    else:
...
def confirm_verification(token):
    """
    Confirm email verification via link click
    """
    # Find user by token
    ...
    user_data['token'] = None  # Clear token after verification
    db.save_user(email, user_data)
...
def check_verification(email):
    ...
    URL Format: /verified/check=user@example.com
    """
    # Validate email
    valid, result = validate_email(email)
    if not valid:
...
if __name__ == '__main__':
    ...
    print("  • Mobile Optimized (max-width: 450px)")
    print("  • Red Verification Button")
    print("  • NO Quoted Text / Hide Quoted Text")
    print("  • No Security Warning")
    print("  • No Fluff/Extra Text")
    print("  • Perfect Phone Screen Fit")
    print("  • 24-Hour Expiry Notice")
    print("  • Auto Base URL Detection")
    print("\n📌 API ENDPOINTS:")
    print("-"*40)
    print("\n1️⃣  Send Verification Link:")
    print(f"   GET /send=user@example.com")
    print("\n2️⃣  Check Verification Status:")
    print(f"   GET /verified/check=user@example.com")
    print("\n3️⃣  View All Users:")
    print(f"   GET /users")
    print("\n" + "="*70)
